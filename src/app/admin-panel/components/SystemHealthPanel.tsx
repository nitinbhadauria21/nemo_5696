  ...
  return (
    <div className="card-surface p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-sans text-muted-foreground">Last checked 30s ago</span>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5">
                <span
                  className={`w-2 h-2 rounded-full ${style.dot} ${svc.status === 'operational' ? 'live-pulse' : ''}`}
                />
                <span className="text-sm font-sans font-medium text-foreground">{svc.name}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs font-mono-custom text-muted-foreground tabular-nums">
                  {svc.latency}
                </span>
                <span className="text-xs font-mono-custom text-muted-foreground tabular-nums">
                  {svc.uptime}
                ...
                <span
                  className={`text-xs font-mono-custom font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${style.badge}`}
                >
                  {style.label}
                </span>
              </div>
