...
const PLATFORMS = [
  { id: 'ca-google', name: 'Google Trends', description: 'Real-time trend data · No API key required', status: 'live', icon: '🔍' },
  { id: 'ca-youtube', name: 'YouTube Data API v3', description: 'Video trends & shorts data', status: 'needs-key', icon: '▶️' },
  { id: 'ca-instagram', name: 'Instagram Graph API', description: 'Reels trends & creator signals', status: 'needs-key', icon: '📸' },
  { id: 'ca-linkedin', name: 'LinkedIn API', description: 'Professional content trends', status: 'needs-key', icon: '💼' },
  { id: 'ca-tiktok', name: 'TikTok API', description: 'Short-form video trends · Coming soon', status: 'coming-soon', icon: '🎵' },
];
...
const STATUS_CONFIG: Record<string, { label: string; badge: string; action: string }> = {
  ...
  'needs-key': { label: 'Connect', badge: 'bg-secondary/10 text-secondary border-secondary/20', action: 'Add API Key' },
  'coming-soon': { label: 'Soon', badge: 'bg-muted text-muted-foreground border-border', action: '' },
  're-auth': { label: 'Re-auth', badge: 'bg-red-500/10 text-red-400 border-red-500/20', action: 'Reconnect' },
};
    ...
    <div className="space-y-4">
      <div className="card-surface p-4 border-secondary/20 bg-secondary/5">
        <p className="text-xs font-sans text-secondary leading-relaxed">
          <span className="font-bold">Platform data is user-provided.</span> Connect your own API keys to fetch real-time data from YouTube, Instagram, and LinkedIn. Google Trends runs without a key.
        </p>
      </div>
            ...
            <div key={platform.id} className="card-surface p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-sans font-semibold text-foreground">{platform.name}</p>
                    <p className="text-xs text-muted-foreground font-sans">{platform.description}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className={`text-xs font-mono-custom font-bold px-2.5 py-1 rounded-full border ${statusCfg.badge}`}>
                    {statusCfg.label}
                  </span>
                  {statusCfg.action && (
                    <button
