    ...
    <div className="space-y-4">
      <div className="card-surface p-4 border-secondary/20 bg-secondary/5">
        <p className="text-sm text-secondary leading-relaxed">
          <span className="font-semibold">Platform data is user-provided.</span> Connect your own
          API keys to fetch real-time data from YouTube, Instagram, and LinkedIn. Google Trends runs
          without a key.
        </p>
      </div>
            ...
            <div key={platform.id} className="card-surface p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span
                    className={`text-[0.75rem] font-semibold px-2.5 py-1 rounded-full border ${statusCfg.badge}`}
                  >
                    {statusCfg.label}
                  </span>
                  {statusCfg.action && (
              ...
              {showInput === platform.id && (
                <div className="mt-3 pt-3 border-t border-border flex gap-2 animate-fade-in">
                  <input
                    ...
                    className="flex-1 bg-input border border-border rounded-xl px-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                  <button
