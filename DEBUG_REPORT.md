...
**Remaining blockers:**
1. Staging Razorpay **test-mode** checkout + webhook end-to-end evidence (needs live test keys — not inventable here)
2. Automated or scripted User A/B RLS proof in CI (or signed-off staging SQL audit refresher)
3. Full browser journey + Lighthouse a11y evidence (Playwright/manual) — focused a11y only so far
4. Operator env + Auth redirect configuration on Vercel (checklist above)
5. Optional hardening: shared Redis/KV rate limit before horizontal scale-out
